# S3

Execute S3 operations including upload, download, list, and delete. Supports AWS S3 and S3-compatible services such as MinIO, Google Cloud Storage (via S3 API), DigitalOcean Spaces, and Backblaze B2.

## DAG-Level Configuration

Configure S3 settings at the DAG level to share connection details across all S3 steps:

```yaml
# DAG-level S3 configuration
s3:
  region: us-east-1
  endpoint: https://s3.amazonaws.com  # Optional for AWS, required for S3-compatible services
  access_key_id: ${env.AWS_ACCESS_KEY_ID}
  secret_access_key: ${env.AWS_SECRET_ACCESS_KEY}
  bucket: my-bucket
  force_path_style: false  # Set true for MinIO and most S3-compatible services

steps:
  # All S3 steps inherit DAG-level configuration

  - id: upload_report
    action: s3.upload
    with:
      key: reports/daily.csv
      source: /tmp/report.csv

  - id: download_config
    action: s3.download
    with:
      key: config/settings.json
      destination: /tmp/settings.json
```

## Step-Level Configuration

```yaml
steps:
  - id: upload_to_s3
    action: s3.upload
    with:
      region: us-west-2
      access_key_id: ${env.AWS_ACCESS_KEY_ID}
      secret_access_key: ${env.AWS_SECRET_ACCESS_KEY}
      bucket: my-bucket
      key: data/file.txt
      source: /local/path/file.txt
```

## Operations

### Upload

Upload a local file to S3:

```yaml
steps:
  - id: upload_file
    action: s3.upload
    with:
      bucket: my-bucket
      key: uploads/document.pdf
      source: /tmp/document.pdf
      content_type: application/pdf  # Optional, auto-detected from extension
      storage_class: STANDARD_IA     # Optional
      metadata:                     # Optional custom metadata
        author: dagu
        version: "1.0"
      tags:                         # Optional object tags
        environment: production
```

### Download

Download an S3 object to local filesystem:

```yaml
steps:
  - id: download_file
    action: s3.download
    with:
      bucket: my-bucket
      key: data/export.csv
      destination: /tmp/export.csv
```

### List

List objects in a bucket:

```yaml
steps:
  - id: list_objects
    action: s3.list
    with:
      bucket: my-bucket
      prefix: logs/2024/        # Optional prefix filter
      max_keys: 100              # Optional, default 1000
      recursive: true           # Optional, default false
      output_format: json        # json (default) or jsonl for streaming
    output: OBJECTS
```

### Delete

Delete a single object or multiple objects by prefix:

```yaml
steps:
  # Delete single object
  - id: delete_file
    action: s3.delete
    with:
      bucket: my-bucket
      key: temp/old-file.txt

  # Delete by prefix (batch delete)
  - id: cleanup_logs
    action: s3.delete
    with:
      bucket: my-bucket
      prefix: logs/2023/
      quiet: true  # Suppress output
```

## Configuration Reference

### DAG-Level Fields

| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `region` | No | - | AWS region (e.g., `us-east-1`) |
| `endpoint` | No | AWS S3 | Custom endpoint URL, including an optional path prefix, for S3-compatible services |
| `access_key_id` | No | - | AWS access key ID |
| `secret_access_key` | No | - | AWS secret access key |
| `session_token` | No | - | AWS session token for temporary credentials |
| `profile` | No | - | AWS credentials profile name |
| `bucket` | No | - | Default bucket name |
| `force_path_style` | No | `false` | Use path-style addressing (required for most S3-compatible services) |
| `disable_ssl` | No | `false` | Disable SSL (for local testing only) |

### Step-Level Fields

All DAG-level fields can be overridden at the step level, plus:

| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `key` | Varies | - | S3 object key (required for upload/download/delete) |
| `source` | Upload | - | Local file path to upload |
| `destination` | Download | - | Local file path for download |
| `prefix` | No | - | Key prefix for list/delete operations |
| `content_type` | No | Auto-detect | Content-Type for uploaded objects |
| `storage_class` | No | `STANDARD` | Storage class (STANDARD, STANDARD_IA, GLACIER, etc.) |
| `metadata` | No | - | Custom metadata key-value pairs |
| `tags` | No | - | Object tags as key-value pairs |
| `max_keys` | No | 1000 | Max keys to return for list |
| `recursive` | No | `false` | List objects recursively |
| `delimiter` | No | - | Delimiter for grouping keys |
| `output_format` | No | `json` | Output format: `json` or `jsonl` |
| `quiet` | No | `false` | Suppress output for delete |

## S3-Compatible Services

Custom endpoints may include a path prefix. Dagu preserves the prefix when constructing and signing S3 requests.

### Supabase

Supabase exposes its S3-compatible API at a path-prefixed endpoint:

```yaml
s3:
  endpoint: "${env.SUPABASE_API_URL}/storage/v1/s3"
  region: "${env.SUPABASE_S3_REGION}"
  access_key_id: "${env.SUPABASE_S3_ACCESS_KEY_ID}"
  secret_access_key: "${env.SUPABASE_S3_SECRET_ACCESS_KEY}"
  bucket: my-bucket
  force_path_style: true

steps:
  - id: upload_to_supabase
    action: s3.upload
    with:
      key: data/file.txt
      source: /tmp/file.txt
```

### MinIO

```yaml
s3:
  endpoint: http://localhost:9000
  access_key_id: minioadmin
  secret_access_key: minioadmin
  bucket: my-bucket
  force_path_style: true  # Required for MinIO
  disable_ssl: true      # If using HTTP

steps:
  - id: upload_to_minio
    action: s3.upload
    with:
      key: data/file.txt
      source: /tmp/file.txt
```

### Google Cloud Storage

GCS supports the S3 API via HMAC keys:

```yaml
s3:
  endpoint: https://storage.googleapis.com
  access_key_id: ${env.GCS_HMAC_ACCESS_KEY}
  secret_access_key: ${env.GCS_HMAC_SECRET}
  bucket: my-gcs-bucket

steps:
  - id: upload_to_gcs
    action: s3.upload
    with:
      key: data/file.txt
      source: /tmp/file.txt
```

To create HMAC keys for GCS:
1. Go to Cloud Storage > Settings > Interoperability
2. Create a key for your service account

### DigitalOcean Spaces

```yaml
s3:
  endpoint: https://nyc3.digitaloceanspaces.com
  access_key_id: ${env.DO_SPACES_KEY}
  secret_access_key: ${env.DO_SPACES_SECRET}
  bucket: my-space
  force_path_style: false  # Spaces uses virtual-hosted style

steps:
  - id: upload_to_spaces
    action: s3.upload
    with:
      key: uploads/file.txt
      source: /tmp/file.txt
```

## Authentication

### Static Credentials

```yaml
s3:
  access_key_id: ${env.AWS_ACCESS_KEY_ID}
  secret_access_key: ${env.AWS_SECRET_ACCESS_KEY}
```

### AWS Profile

```yaml
s3:
  profile: production
```

### IAM Role (EC2/ECS/Lambda)

When running on AWS infrastructure with an IAM role, credentials are automatically discovered:

```yaml
s3:
  region: us-east-1
  bucket: my-bucket
# No credentials needed - uses instance role
```

## Variable Expansion

Scoped references in S3 configuration fields (`access_key_id`, `secret_access_key`, `endpoint`, `bucket`, etc.) expand DAG-scoped values. OS environment variables such as `$HOME` are **not** expanded directly. To use an OS value, import it in the `env:` block:

```yaml
env:
  - AWS_KEY: ${AWS_ACCESS_KEY_ID}       # Import from OS environment
  - AWS_SECRET: ${AWS_SECRET_ACCESS_KEY}

s3:
  access_key_id: ${env.AWS_KEY}
  secret_access_key: ${env.AWS_SECRET}
  bucket: my-bucket
```

## Output Format

### Upload Result

```json
{
  "operation": "upload",
  "success": true,
  "bucket": "my-bucket",
  "key": "data/file.txt",
  "uri": "s3://my-bucket/data/file.txt",
  "etag": "\"d41d8cd98f00b204e9800998ecf8427e\"",
  "size": 1024,
  "contentType": "text/plain",
  "storageClass": "STANDARD",
  "duration": "125ms"
}
```

### Download Result

```json
{
  "operation": "download",
  "success": true,
  "bucket": "my-bucket",
  "key": "data/file.txt",
  "uri": "s3://my-bucket/data/file.txt",
  "destination": "/tmp/file.txt",
  "size": 1024,
  "contentType": "text/plain",
  "etag": "\"d41d8cd98f00b204e9800998ecf8427e\"",
  "duration": "89ms"
}
```

### List Result

```json
{
  "operation": "list",
  "success": true,
  "bucket": "my-bucket",
  "prefix": "logs/",
  "objects": [
    {
      "key": "logs/app.log",
      "size": 2048,
      "lastModified": "2024-01-15T10:30:00Z",
      "etag": "\"abc123\"",
      "storageClass": "STANDARD"
    }
  ],
  "totalCount": 1,
  "duration": "50ms"
}
```

### Delete Result

```json
{
  "operation": "delete",
  "success": true,
  "bucket": "my-bucket",
  "key": "temp/old-file.txt",
  "deletedCount": 1,
  "deletedKeys": ["temp/old-file.txt"],
  "duration": "45ms"
}
```

## See Also

- [HTTP](/step-types/http) - HTTP requests
- [Docker](/step-types/docker) - Container execution
